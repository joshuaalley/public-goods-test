...
Per... the expected value of the defense burden is equal to:
\begin{equation}
...
Moreover, the implications of this non-linear relationship for inference depend on autocorrelation in time-series of GDP and defense spending within states. 
The GDP and defense spending values that Olson and Zeckhauser, as well as other researchers e.g.\citep{Oneal1990, KimSandler2019} employ in cross-sectional correlations depend on past GDP and military spending values. 
At the very least, simultaneous increases over time can lead to correlations between GDP and military spending within units.  
If GDP and military spending decisions are correlated for other reasons, this further complicates the ratio calculations. 


In what follows, I report the results of two simulation analyses to assess how using defense burdens might affect inferences.  
This simulation approximates a rough approximation of common tests of burden-sharing within NATO, which estimate annual correlations between GDP and defense burdens among NATO members. 
In both simulations, I simulate an outcome and independent variable, both of which have strong temporal autocorrelation. 
I then analyze cross-sectional associations between the independent variable and the outcome with and without taking the outcome as a share of the independent variable. 
I set the values of the outcome and independent variable to roughly match the distribution of military spending burdens. 
The data-generating process contains 100 temporal observations of 20 units. 
Therefore, when I run 100 cross-sectional analyses, all the linear regression estimates have 18 degrees of freedom after estimating a regression coefficient and intercept.  

 
In the first simulation, I assume that the independent variable and outcome are entirely uncorrelated, except for any correlation from time trends. 
In the second simulation, I assume that the independent variable and outcome are correlated and could have common time trends. 
I set the assumed correlation equal to the observed correlation between GDP and military spending in my data, which is .48. 
...
When the independent variable and outcome are independent, creating a ratio of the two generates an overwhelming number of statistically significant findings. 
When the independent variable and dependent variable are correlated, the ratio variable has a different effect, however. 
...
Based on a chi-squared test, there is a statistically significant differences in the inferences under ratio and non-ratio outcomes for both types of simulated variables.


\begin{figure}
	\centering
		\includegraphics[width=0.95\textwidth]{sim-inferences.pdf}
...
But if the two variables are correlated, creating a ratio is more likely to produce findings that do not meet conventional statistical significance thresholds.
Therefore, research designs that examine the correlation between GDP and military spending as a share of GDP may produce misleading findings, including null results. 
