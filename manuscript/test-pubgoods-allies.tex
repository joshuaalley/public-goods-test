...
Because \textbf{Z} contains each state's share of allied GDP or economic weight in an alliance, these coefficients estimate the association between a state's economic weight in an alliance and military spending.\footnote{This assumes symmetric effects across small and large states, which I relax in the appendix with a weighted coding of $\textbf{Z}$.} 
...
Predictions from the model about specific alliances add further evidence of little free-riding based on economic weight. 
\autoref{tab:pred-sum} summarizes predictions from the model estimates about several notable alliances. 
Generally, there is little evidence of extensive free-riding by small and large states. 


\begin{table}[hbt!]
\begin{center}
\begin{tabular}{cccc}
   Alliance  & Small Member Impact & Large Member Impact  \\
\hline
 NATO           & 0.003 & -0.003   \\
 US-South Korea &        &          \\ 
 Arab League    &        &        \\
\hline
\end{tabular}
\caption{Table of treaty depth estimates, year of formation and sources of depth for four alliances. }
\label{tab:pred-sum}
\end{center} 
\end{table}
...
The posterior mean of this parameter is -.003, and it has 81\% \emph{negative} posterior mass, so differences in economic weight do not drive differences in spending between NATO members.  
